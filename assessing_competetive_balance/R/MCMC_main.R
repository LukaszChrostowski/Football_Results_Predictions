...
mcmc <- function(season, N, y, O) {
  ...
  obj <- label_correction(Z_seq_burned = Z_seq_burned,
                          K_max = K_max,
                          True_K_seq_burned = True_K_seq_burned,
                          O = O,
                          K_seq = K_seq)
  K_estimated <- obj$K_estimated
  Ordered_Tabellone <- obj$table
  Noisy_True_K <- obj$Noisy_True_K
  K_maxmax <- obj$K_maxmax
  K_minmin <- obj$K_minmin
  #Team_Names <- cbind(colnames(O), colnames(O))

  list(O = O,
       season_lab = season_lab,
       Noisy_True_K = Noisy_True_K,
       K_maxmax = K_maxmax,
       table = Ordered_Tabellone,
       N = N,
       K_estimated = K_estimated,
       Z_seq_burned = Z_seq_burned,
       S = S,
       K_seq = K_seq
       )
}
